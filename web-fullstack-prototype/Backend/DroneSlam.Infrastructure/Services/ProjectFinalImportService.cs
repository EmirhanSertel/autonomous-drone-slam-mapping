    ...
    private static class MapCatalog
    {
        public static MapMetadata Get(string mapKey)
        {
            return Catalog.TryGetValue(mapKey, out var metadata)
                ? metadata
                : new MapMetadata(mapKey.ToUpperInvariant(), "Otomatik içe aktarılan SLAM koşusu", "Yeni", "warn", "-", "Project-Final klasöründen otomatik tespit edildi.", null, null, null, null, null, ["Otomatik import ile eklendi."]);
        }
