#!/usr/bin/env bash
# Maps8 ortam kurulumu — Maps1 / slam_ws ile cakisma yok
# Maps8/slam_ws kendi install/'ini kullanir
set -euo pipefail

MAPS8_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MAPS8_WS="${MAPS8_ROOT}/slam_ws"
...
# Maps8 kendi workspace'i — Maps1'den (~/slam_ws) BAGIMSIZ
if [ -f "${MAPS8_WS}/install/setup.bash" ]; then
  set +u; source "${MAPS8_WS}/install/setup.bash"; set -u
else
  echo "UYARI: Maps8 workspace henuz build edilmemis. Once ./scripts/build.sh calistir." >&2
fi

export MAPS8_ROOT MAPS8_WS
